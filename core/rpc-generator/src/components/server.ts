    ...
    afterCompileTemplate: [
        async (
            ...
            for (const method of dereffedDocument.methods as MethodObject[]) {
                ...
                const functionAliasName = typings.getTypingNames(
                    'typescript',
                    method
                ).method
                ...
                let codeToWrite = ''
                if (exists) {
                    const existingMethod = await readFile(
                        methodFileName,
                        'utf8'
                    )
                    /* eslint-disable-next-line */
                    const methodRegExp = new RegExp(
                        `const ${method.name}: ${functionAliasName} = \(.*\) =>`,
                        'gm'
                    )
