    ...
    afterCompileTemplate: [
        async (dest, frm, component, openrpcDocument): Promise<void> => {
            if (component.language === 'typescript') {
                ...
                // Step 2: Strip AnyOf union type from generated .ts files
                const tsFiles = await glob(`${dest}/**/*.ts`)
                for (const file of tsFiles) {
                    const content = await fs.promises.readFile(file, 'utf8')
                    const cleaned = stripAnyOfTypes(content)
                    await fs.promises.writeFile(file, cleaned)
                }

                // Step 3: Prettify the output
                execSync(`yarn prettier --write ${dest}/**/*.ts`)
            }
