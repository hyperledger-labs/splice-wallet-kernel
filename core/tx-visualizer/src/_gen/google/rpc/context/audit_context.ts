...
class AuditContext$Type extends MessageType<AuditContext> {
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: AuditContext): AuditContext {
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* google.protobuf.Struct scrubbed_request */ 2:
                    message.scrubbedRequest = Struct.internalBinaryRead(reader, reader.uint32(), options, message.scrubbedRequest);
                    break;
                case /* google.protobuf.Struct scrubbed_response */ 3:
                    message.scrubbedResponse = Struct.internalBinaryRead(reader, reader.uint32(), options, message.scrubbedResponse);
                    break;
    ...
    internalBinaryWrite(message: AuditContext, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* bytes audit_log = 1; */
        ...
        if (message.scrubbedRequest)
            Struct.internalBinaryWrite(message.scrubbedRequest, writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        /* google.protobuf.Struct scrubbed_response = 3; */
        if (message.scrubbedResponse)
            Struct.internalBinaryWrite(message.scrubbedResponse, writer.tag(3, WireType.LengthDelimited).fork(), options).join();
        /* int32 scrubbed_response_item_count = 4; */
        if (message.scrubbedResponseItemCount !== 0)
            writer.tag(4, WireType.Varint).int32(message.scrubbedResponseItemCount);
        /* string target_resource = 5; */
        if (message.targetResource !== "")
            writer.tag(5, WireType.LengthDelimited).string(message.targetResource);
