...
export class InternalKeystore {
    ...
    public internalKeystoreController = buildController({
        signTransaction: async (
            params: SignTransactionParams
        ): Promise<SignTransactionResult> => {
            // validate the transaaction here
            const key = this.keystoreByPublicKey.get(params.publicKey)
            if (key) {
                const txId = randomUUID()
                const decodedKey = naclUtil.decodeBase64(key.privateKey)
                const signature = naclUtil.encodeUTF8(
                    nacl.sign(naclUtil.decodeUTF8(params.txHash), decodedKey)
                )
