...
| `e2e-affected`        | Affected package check              | Always                                                  | None                                                                | computes suite-specific outputs: `run_tests_ping`, `run_tests_portfolio`, `run_tests_wallet_sdk` plus matched project lists |
| `test-ping-e2e`       | E2E (ping app)                      | Conditional                                             | `needs: [build, e2e-affected]` and `run_tests_ping == 'true'`       | starts Canton+services and runs Playwright for `@canton-network/example-ping`                                               |
| `test-portfolio-e2e`  | E2E (portfolio app)                 | Conditional                                             | `needs: [build, e2e-affected]` and `run_tests_portfolio == 'true'`  | starts Canton+services and runs Playwright for `@canton-network/example-portfolio`                                          |
| `sdk-e2e`             | Wallet SDK E2E (slow)               | Conditional                                             | `needs: [build, e2e-affected]` and `run_tests_wallet_sdk == 'true'` | snippet + example script tests on matrix `devnet` + `mainnet`                                                               |
